...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const [newChatbotForm, setNewChatbotForm] = useState<NewChatbotForm>({
    name: '',
    goal: '',
    description: '',
      ...
      <div className="absolute top-20 left-0 right-0 z-40 px-4 sm:px-6 py-4 border-b border-border/50 bg-background/95 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="hidden sm:block text-xs sm:text-sm text-muted-foreground">Build websites and tools with AI-powered automation</p>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="gap-2 border-border/50 bg-card/50 text-xs sm:text-sm flex-1 sm:flex-none">
                  ...
                  <span className="truncate max-w-[100px] sm:max-w-[150px]">{models.find(m => m.id === selectedModel)?.name.split(':')[0] || 'Model'}</span>
                  <ChevronDown className="h-3.5 w-3.5 opacity-50 hidden sm:block" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-64 text-xs sm:text-sm">
                {models.map(model => (
                  <DropdownMenuItem 
                    key={model.id} 
